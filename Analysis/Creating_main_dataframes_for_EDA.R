...
#prs_filenames <- list.files("/Users/amink/OneDrive/Documents/Current Jobs/Masters Thesis/Code/Datasets/PRS_PRSice_No_MHC/",full.names = T) ; no_mhc = TRUE #|
                                                                                                                                                            #|
#prs_filenames <- list.files("/Users/amink/OneDrive/Documents/Current Jobs/Masters Thesis/Code/Datasets/CLUMP_200_0.25/PRS_PLINK/",full.names = T) ; no_mhc = FALSE        #|
prs_filenames <- list.files("/Users/amink/OneDrive/Documents/Current Jobs/Masters Thesis/Code/Datasets/CLUMP_200_0.25/PRS_PLINK_No_MHC/",full.names = T) ; no_mhc = TRUE #|
                                                                                                                                                            #|
#---------------------------------------------------------------------------------------------------------------------------------------------------------- #
...
for (thresh in dsets_torun) {
                ...
                #                                                                           ] > 99)]
                # 
                # res.clean <- as.matrix(as.data.frame(res.clean)[,which(names(as.data.frame(res.clean)) %in% pheno_passed)])
                ...
                for (i in 1:dim(check)[1]){
                                ...
                                if((corr$p.value <0.05/dim(matrix[2])) & (abs(corr$estimate)>0.95)){
                                                ...
                                                } else if (grepl( "PH", matrix[check[i,1]], fixed = TRUE)){
                                                                to_remove <- append(to_remove,check[i,2])
                                                } else if (grepl( "PH", matrix[check[i,2]], fixed = TRUE)){
                                                                to_remove <- append(to_remove,check[i,1])
                                                } else{
                ...
                if(isTRUE(no_mhc)){
                                ...
                                #PLINK#
                                saveRDS(allres,file=paste0("/Users/amink/OneDrive/Documents/Current Jobs/Masters Thesis/Code/Datasets/CLUMP_200_0.25/PRS_PLINK_No_MHC_Resid_PCA_Clust/", "PCAresults_",pval,".rds"))
                } else{
                                #PRSice#
                                ...
                                #PLINK#
                                saveRDS(allres,file=paste0("/Users/amink/OneDrive/Documents/Current Jobs/Masters Thesis/Code/Datasets/CLUMP_200_0.25/PRS_PLINK_Resid_PCA_Clust/", "PCAresults_",pval,".rds"))  
                }
}
